...
func UninstallModeOptions() []UninstallModeOption {
	return []UninstallModeOption{
		...
		},
		{
			...
			Description: "Remove all axiom managed configuration from all agents",
		},
		{
			Mode:        model.UninstallModeFullRemove,
			Label:       "Full Uninstall & Remove Binary",
			Description: "Remove all configuration AND delete the axiom binary itself",
		},
		{
...
func RenderUninstallMode(cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Choose how you want to uninstall axiom:"))
	b.WriteString("\n\n")
...
func RenderUninstall(selected []model.AgentID, cursor int) string {
	...
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select the agents whose axiom managed configuration should be removed."))
	b.WriteString("\n\n")
...
func RenderUninstallComponents(selected []model.ComponentID, cursor int) string {
	...
	b.WriteString(styles.HelpStyle.Render("Use j/k to move, space to toggle, enter to continue."))
	b.WriteString("\n\n")
	b.WriteString(styles.SubtextStyle.Render("Select which axiom managed components should be removed from the selected agents."))
	b.WriteString("\n\n")
...
func RenderUninstallConfirm(mode model.UninstallMode, selected []model.AgentID, components []model.ComponentID, profilesToRemove []string, engramScope model.EngramUninstallScope, engramProjectScopeAvailable bool, cursor int, operationRunning bool, spinnerFrame int) string {
	...
	case model.UninstallModeFull:
		...
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all axiom managed configuration from all supported agents."))
		b.WriteString("\n")
	case model.UninstallModeFullRemove:
		b.WriteString(styles.ErrorStyle.Render("Mode: Full Uninstall & Remove Binary"))
		b.WriteString("\n\n")
		b.WriteString(styles.UnselectedStyle.Render("This will remove all axiom managed configuration from all agents"))
		b.WriteString("\n")
		b.WriteString(styles.ErrorStyle.Render("AND delete the axiom binary itself."))
		b.WriteString("\n\n")
	...
	if err != nil {
		...
		if mode == model.UninstallModeCleanInstall {
			...
			if syncErr != nil {
				...
				b.WriteString("\n\n")
				b.WriteString(styles.WarningStyle.Render("You can run 'axiom sync' manually to retry."))
			} else {
