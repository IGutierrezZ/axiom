...
func TestInstallScriptBetaGoInstallBypassesPublicGoProxy(t *testing.T) {
	...
	for _, clobber := range []string{
		"GONOSUMDB=github.com/IGutierrezZ/axiom/v3 \\",
		"GOPRIVATE=github.com/IGutierrezZ/axiom/v3 \\",
...
`)
	...
	want := strings.Join([]string{
		"github.com/IGutierrezZ/axiom/v3,example.com/private",
		"github.com/IGutierrezZ/axiom/v3,github.com/acme/*",
		"github.com/IGutierrezZ/axiom/v3",
	}, "\n")
	if got != want {
...
func TestWindowsInstallScriptBetaGoInstallPreservesGoProxyBypassEnv(t *testing.T) {
	...
	for _, clobber := range []string{
		"$env:GONOSUMDB = \"github.com/IGutierrezZ/axiom/v3\"",
		"$env:GOPRIVATE = \"github.com/IGutierrezZ/axiom/v3\"",
		"$env:GONOPROXY = \"github.com/IGutierrezZ/axiom/v3\"",
	} {
		if strings.Contains(script, clobber) {
...
func TestInstallScriptsGoInstallPackageMatchesModuleMajor(t *testing.T) {
	...
	majorMatch := regexp.MustCompile(`^module github\.com/IGutierrezZ/axiom/(v[0-9]+)$`).FindStringSubmatch(strings.TrimSpace(moduleLine))
	if majorMatch == nil {
