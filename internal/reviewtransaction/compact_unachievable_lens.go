...
func (store CompactStore) RecordUnachievableLensAttempt(ctx context.Context, request CompactUnachievableLensAttemptRequest) (recorded bool, err error) {
	...
	if state.State != StateReviewing || state.CapturePhaseRevision != request.ExpectedRevision ||
		...
		return false, errors.New("unachievable lens attempt does not match the current reviewing authority; refresh the binding with axiom review status --cwd <repo> --contract axiom.review-integration/v2 --next-transition")
	}
	if _, found, activeErr := state.ActiveAdmittedLensResult(request.SelectedOrder); activeErr != nil {
		return false, activeErr
	} else if found {
		return false, errors.New("this lens slot already holds a captured reviewer result; run axiom review status --cwd <repo> --contract axiom.review-integration/v2 --next-transition to see its admitted artifact")
	}
	for _, existing := range state.UnachievableLensAttempts {
		...
		return false, errors.New("a different unachievable declaration already exists for this lens slot; refresh the negotiated collection with axiom review status --cwd <repo> --contract axiom.review-integration/v2 --next-transition")
	}
...
func (store CompactStore) WithdrawUnachievableLensAttempt(ctx context.Context, request CompactUnachievableLensWithdrawalRequest) (removed bool, err error) {
	...
	if state.State != StateReviewing || state.CapturePhaseRevision != request.ExpectedRevision || state.InitialSnapshot.Identity != request.TargetIdentity {
		return false, errors.New("unachievable lens withdrawal does not match the current reviewing authority; refresh the binding with axiom review status --cwd <repo> --contract axiom.review-integration/v2 --next-transition")
	}
	index := -1
	...
	if _, found, activeErr := state.ActiveAdmittedLensResult(order); activeErr != nil {
		...
		return false, errors.New("this lens slot now holds a captured reviewer result; run axiom review status --cwd <repo> --contract axiom.review-integration/v2 --next-transition to see its admitted artifact and continue")
	}
	next := cloneCompactStateInitialAtomicStart(state)
