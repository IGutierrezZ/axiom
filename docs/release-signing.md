...
3. Verify the signed identity before trusting any checksum:
   ...
   # Must print exactly:
   # repo=IGutierrezZ/axiom;tag=vMAJOR.MINOR.PATCH
...
```text
github.com/IGutierrezZ/axiom/v3/internal/update/upgrade.releaseMinisignPublicKeys
```
...
- With Go 1.25.10+ on `PATH`, the built-in upgrader runs
  `go install github.com/IGutierrezZ/axiom/v3/cmd/gentle-ai@vX.Y.Z`,
